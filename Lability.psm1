...
$moduleRoot = Split-Path -Path $MyInvocation.MyCommand.Path -Parent;
$modulePrivatePath = Join-Path -Path $moduleRoot -ChildPath 'Private';
$modulePublicPath = Join-Path -Path $moduleRoot -ChildPath 'Public';
Get-ChildItem -Path $modulePrivatePath,$modulePublicPath -Include *.ps1 -Exclude '*.Tests.ps1' -Recurse |
    ForEach-Object {
